package cwchoiit.blackfriday.order.consumer;

import cwchoiit.blackfriday.event.Event;
import cwchoiit.blackfriday.event.payload.EventPayload;
import cwchoiit.blackfriday.order.eventhandler.EventHandler;
...
import static cwchoiit.blackfriday.event.EventType.Topic.ORDER_TO_DELIVERY;
import static cwchoiit.blackfriday.event.EventType.Topic.ORDER_TO_PAYMENT;

@Slf4j
@Component
@RequiredArgsConstructor
public class ToOrderEventConsumer {
    private final List<EventHandler<? extends EventPayload>> eventHandlers;

    @KafkaListener(topics = {ORDER_TO_PAYMENT, ORDER_TO_DELIVERY})
